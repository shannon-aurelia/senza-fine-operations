...
export type CurrentUser = { id: string; name: string; email: string | null; department: string; role: string };
...
export function normalizeSheetData(payload: Record<string, unknown>): SheetData {
  ...
  return {
    ...
    currentUser: payload.currentUser && typeof payload.currentUser === "object" ? {
      id: text((payload.currentUser as Record<string, unknown>).id),
      name: text((payload.currentUser as Record<string, unknown>).name),
      email: text((payload.currentUser as Record<string, unknown>).email) || null,
      department: text((payload.currentUser as Record<string, unknown>).department),
      role: text((payload.currentUser as Record<string, unknown>).role) || "staff",
    } : null,
  };
}
...
export function CatalogView({ data, submit, onRefresh }: { data: SheetData; submit: SubmitAction; onRefresh: () => void }) {
  const [creating, setCreating] = useState(false);
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => data.catalog.filter((item) => [item.category, item.itemName, item.sku, item.brand].join(" ").toLowerCase().includes(query.toLowerCase())), [data.catalog, query]);
  const categoryCount = new Set(data.catalog.map((item) => item.category)).size;
  return <section className="module-page"><div className="module-heading"><div><p className="eyebrow">Master data</p><h2>Items & Categories</h2><p>Create categories such as Beef, Chicken, Vegetables, or Fruits, then add items such as Sirloin or Tenderloin.</p></div><button className="primary-button" onClick={() => setCreating(true)}>+ Create item & category</button></div><div className="mini-kpis"><article><span>Categories</span><strong>{categoryCount}</strong></article><article><span>Items</span><strong>{data.catalog.length}</strong></article></div><div className="filter-bar"><label className="search-field"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search category, item, SKU, or brand"/></label></div><div className="data-table-wrap"><table className="data-table catalog-table"><thead><tr><th>Category</th><th>Item</th><th>SKU</th><th>Brand</th><th>UOM</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td><span className="location-chip">{item.category}</span></td><td><strong>{item.itemName}</strong></td><td>{item.sku || "—"}</td><td>{item.brand || "—"}</td><td>{item.uom}</td></tr>)}</tbody></table></div>{creating ? <CreateItemModal data={data} submit={submit} onClose={() => setCreating(false)} onCreated={onRefresh}/> : null}</section>;
}

export function InventoryView({ data, submit, onRefresh, operatorName, operatorDepartment }: { data: SheetData; submit: SubmitAction; onRefresh: () => void; operatorName: string; operatorDepartment: string }) {
  const [query, setQuery] = useState("");
  ...
  async function stockOut(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await submit({
      action: "stockOut", inventoryId: selected.id, name: operatorName, department: operatorDepartment, location: selected.loc,
...
export function PurchasingView({ data, submit, onRefresh, accessRole = "owner", operatorName = "", operatorDepartment = "", operatorEmail = "" }: { data: SheetData; submit: SubmitAction; onRefresh: () => void; accessRole?: string; operatorName?: string; operatorDepartment?: string; operatorEmail?: string }) {
  ...
  async function submitRequest(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    const form = new FormData(event.currentTarget);
    const result = await submit({ action: "purchaseRequest", name: operatorName, requestedBy: operatorName, department: operatorDepartment, email: operatorEmail, remarks: text(form.get("remarks")), items: validLines.map((line) => ({ type: catalog.find((item) => item.sku === line.sku && item.detail === line.item)?.type || "", sku: line.sku, itemName: line.item, detail: line.item, qtyRequested: number(line.qty), qty: number(line.qty), uom: line.uom, location: line.location })) });
    setStatus({ tone: result.ok ? "success" : "error", message: result.message });
    setSubmitting(false);
  ...
  const visibleGroups = (tab === "Review" ? requestGroups.filter((group) => group.status === "Submitted") : tab === "Approval" ? requestGroups.filter((group) => group.status === "Reviewed") : requestGroups).filter((group) => accessRole === "owner" || (tab === "Review" ? group.designatedReviewer === operatorName : group.requestedBy === operatorName));
  return <section className="module-page">
...
export function UsageView({ data, submit, onRefresh, operatorName, operatorDepartment }: { data: SheetData; submit: SubmitAction; onRefresh: () => void; operatorName: string; operatorDepartment: string }) {
  ...
  return <section className="module-page">
    ...
    <form className="workflow-card simple-workflow" onSubmit={submitUsage}>
      ...
      <div className="form-grid padded-form">
        ...
        <label><span>Used by</span><input value={operatorName} readOnly/></label>
        <label><span>Department</span><input value={operatorDepartment} readOnly/></label>
...
export function ReceivingView({ data, submit, onRefresh, operatorName }: { data: SheetData; submit: SubmitAction; onRefresh: () => void; operatorName: string }) {
  const [query, setQuery] = useState("");
  ...
  return <section className="module-page">
    ...
    {selected ? <div className="modal-backdrop" onMouseDown={() => setSelected(null)}><form className="action-modal" onSubmit={submitReceipt} onMouseDown={(event) => event.stopPropagation()}><div className="modal-heading"><div><p className="eyebrow">Receive PR {selected.prId}</p><h3>{selected.itemName}</h3><span>{Math.max(0, selected.qtyRequested - selected.qtyReceived)} {selected.uom} remaining</span></div><button type="button" onClick={() => setSelected(null)}>×</button></div><div className="form-grid"><label><span>Received by</span><input value={operatorName} readOnly/></label><label><span>Location</span><select name="location" defaultValue={selected.location || "Outlet"}>{data.locs.map((loc) => <option key={loc}>{loc}</option>)}</select></label><label><span>Quantity received</span><input name="qtyReceived" type="number" min="0.01" max={Math.max(0, selected.qtyRequested - selected.qtyReceived)} step="any" required/></label><label><span>Expiry date</span><input name="expiryDate" type="date"/></label><label><span>Package photo</span><input name="packagePhoto" type="file" accept="image/jpeg,image/png,image/webp" required/></label><label><span>Receipt / invoice scan</span><input name="invoicePhoto" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required/></label><label className="wide"><span>Receiving notes</span><textarea name="remarks" rows={3} placeholder="Condition, shortage, supplier note, or batch information"/></label></div><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setSelected(null)}>Cancel</button><button className="primary-button">Confirm receipt and update inventory</button></div></form></div> : null}
  </section>;
}

export function ExpiryView({ data, submit, onRefresh, operatorName }: { data: SheetData; submit: SubmitAction; onRefresh: () => void; operatorName: string }) {
...
export function DailyOperationsView({ data, submit, operatorName, operatorDepartment }: { data: SheetData; submit: SubmitAction; operatorName: string; operatorDepartment: string }) {
...
export function InviteUserView({ submit }: { submit: SubmitAction }) {
  const [status, setStatus] = useState<{ tone: string; message: string } | null>(null);
  const [busy, setBusy] = useState(false);
  async function invite(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setStatus(null);
    const form = new FormData(event.currentTarget);
    const result = await submit({ action: "inviteUser", name: text(form.get("name")), email: text(form.get("email")), department: text(form.get("department")), role: text(form.get("role")), redirectTo: `${window.location.origin}/reset-password` });
    setBusy(false); setStatus({ tone: result.ok ? "success" : "error", message: result.message });
    if (result.ok) event.currentTarget.reset();
  }
  return <section className="module-page"><div className="module-heading"><div><p className="eyebrow">Owner controls</p><h2>Create User & Password</h2><p>Enter an employee name and email. They receive a secure email link to choose their own password.</p></div><span className="live-badge">Owner only</span></div><form className="workflow-card padded-form create-user-card" onSubmit={invite}><div className="form-grid"><label><span>Full name</span><input name="name" required placeholder="Example: Nissa"/></label><label><span>Email</span><input name="email" type="email" required placeholder="nissa@email.com"/></label><label><span>Department</span><select name="department" defaultValue="Floor"><option>Floor</option><option>Kitchen</option><option>Utilities</option><option>Owner</option></select></label><label><span>Access role</span><select name="role" defaultValue="staff"><option value="staff">Staff</option><option value="reviewer">Reviewer</option><option value="owner">Owner</option></select></label></div><div className="reviewer-help"><strong>Reviewer assignments</strong><span>Floor: Nissa · Kitchen: Yasri · Owner: can review and approve all departments.</span></div><StatusBanner status={status}/><div className="form-footer"><span>The password is private and is never shown to the Owner.</span><button className="primary-button" disabled={busy}>{busy ? "Sending invitation…" : "Create user & send password link"}</button></div></form></section>;
}
