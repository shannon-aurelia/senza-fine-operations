...
export function UsersView({ data, submit, onRefresh }: { data: SheetData; submit: SubmitAction; onRefresh: () => void }) {
  ...
  function generatePassword() {
    ...
    setPassword(`Sf!${[...bytes].map((byte) => byte.toString(16).padStart(2, "0")).join("")}9`);
  }
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!selected) return; setBusy(true);
    const form = new FormData(event.currentTarget);
    const result = await submit({ action: "saveUser", name: selected.name, email: text(form.get("email")), department: text(form.get("department")), role: text(form.get("role")), password, active: form.get("active") === "on" });
    setBusy(false); setStatus({ tone: result.ok ? "success" : "error", message: result.message });
    if (result.ok) { setSelected(null); setPassword(""); onRefresh(); }
  }
  return <section className="module-page"><div className="module-heading"><div><p className="eyebrow">Owner controls</p><h2>Users and permissions</h2><p>Give every employee their own email and temporary password. Pending names cannot sign in until the Owner activates them.</p></div><span className="live-badge">Owner only</span></div><StatusBanner status={status}/><div className="account-grid">{data.users.map((user) => <article key={user.id}><div className="account-avatar">{user.name.slice(0,1).toUpperCase()}</div><div className="account-copy"><span>{user.department} · {user.role}</span><h3>{user.name}</h3><p>{user.email || "Email not assigned"}</p><small>{user.active ? user.last_login_at ? `Last login ${formatDate(user.last_login_at)}` : "Active · has not signed in" : "Pending setup"}</small></div><span className={user.active ? "risk safe" : "risk warning"}>{user.active ? "Active" : "Pending"}</span><button className="row-button" onClick={() => { setSelected(user); setPassword(""); setStatus(null); }}>{user.active ? "Edit" : "Set up"}</button></article>)}</div>{selected ? <div className="modal-backdrop" onMouseDown={() => setSelected(null)}><form className="action-modal" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}><div className="modal-heading"><div><p className="eyebrow">Staff account</p><h3>{selected.name}</h3><span>Individual login and permissions</span></div><button type="button" onClick={() => setSelected(null)}>×</button></div><div className="form-grid"><label className="wide"><span>Email</span><input name="email" type="email" defaultValue={selected.email || ""} required/></label><label><span>Department</span><select name="department" defaultValue={selected.department}><option>Owner</option><option>Floor</option><option>Kitchen</option><option>Utilities</option></select></label><label><span>Role</span><select name="role" defaultValue={selected.role}><option value="staff">Staff</option><option value="reviewer">Reviewer</option><option value="owner">Owner</option></select></label><label className="wide"><span>{selected.active ? "New temporary password (leave blank to keep current)" : "Temporary password"}</span><div className="password-row"><input value={password} onChange={(event) => setPassword(event.target.value)} minLength={10} required={!selected.active} placeholder="At least 10 characters"/><button type="button" className="secondary-button" onClick={generatePassword}>Generate</button></div></label><label className="wide active-check"><input name="active" type="checkbox" defaultChecked={selected.active}/><span>Account is active and may sign in</span></label></div><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setSelected(null)}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? "Saving…" : "Save account"}</button></div></form></div> : null}</section>;
}
